...
import 'critical_payment_api_provider.dart';

class CriticalPaymentController extends PaymentController {

  CriticalPaymentController(
  ...
  void _handleRazorPayPaymentSuccess(PaymentSuccessResponse response) async {
    ...
    debugPrint(
        "RENEWAL PAYMENT SUCCESS FROM GATEWAY -> Payment id: ${response
            .paymentId} | Order id: ${response.orderId} | Signature: ${response
            .signature}");
    debugPrint(
        "RENEWAL PAYMENT SUCCESS FROM GATEWAY -> Payment id: ${_response
            .paymentId} | Order id: ${_response.orderId} | Signature: ${_response
            .signature}");
    ...
    callCriticalPaymentVerificationApi();
  }


  void callCriticalPaymentVerificationApi() async {
    setCurrentStep = PaymentSteps.PAYMENT_VERIFICATION;
    if (_paymentInsuranceOldRequest != null) {
      ParsedResponse<InsurancePaymentResModel> response = await CriticalIllnessPaymentApiProvider()
          .criticalIllnessPaymentStatusCheckApiCall(
