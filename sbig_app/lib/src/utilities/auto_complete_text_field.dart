...
class AutoCompleteTextFieldState<T> extends State<AutoCompleteTextField> {
  ...
  void updateOverlay([String query]) {
    if (listSuggestionsEntry == null) {
      ...
      final width = textFieldSize.width + 20;
                                  ...
                                  onTap: () {
                                    setState(() {
                                      ...
                                      if (submitOnSuggestionTap) {
                                        //String newText = ' ${suggestion?.area ??''} ${suggestion?.city ?? ''} ${suggestion?.state??''}';
  ...
  List<T> getSuggestions(List<T> suggestions, Comparator<T> sorter,
    ...
    //suggestions = suggestions.where((item) => filter(item.state, query) && filter(item.pincode,query) && filter(item.city, query) && filter(item.area, query) ).toList();
...
class SimpleAutoCompleteTextField extends AutoCompleteTextField<String> {
  ...
  State<StatefulWidget> createState() => new AutoCompleteTextFieldState<String>(
          ...
          itemSubmitted,
          (context, item) {
            print('SUGGESTION ITEM called : $item');
            return Padding(
          ...
          (item, query) {return item.toLowerCase().startsWith(query.toLowerCase());},
          suggestionsAmount,
          submitOnSuggestionTap,
          clearOnSubmit,
          minLength,
          ...
          (context, item) {
            ...
            return  Padding(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                     Expanded(child: Text(item)),
                    Icon(Icons.call_made,color: Colors.black,),
                  ],
                ),
            );
            },
