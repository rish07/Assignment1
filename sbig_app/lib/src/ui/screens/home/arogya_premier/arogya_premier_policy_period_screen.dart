...
import 'arogya_premier_insure_details_screen.dart';

class ArogyaPremierPolicyPeriodScreen extends StatefulWidgetBase {

  static const ROUTE_NAME = "/arogya/policy_period_screen";

  final ArogyaPremierModel arogyaPremierModel;

  ArogyaPremierPolicyPeriodScreen(this.arogyaPremierModel);

  @override
  _PolicyPeriodScreenState createState() => _PolicyPeriodScreenState(policyMembers: arogyaPremierModel.policyMembers);
}

class _PolicyPeriodScreenState extends State<ArogyaPremierPolicyPeriodScreen>
    with CommonWidget {
  ...
  @override
  void initState() {
    years = widget.arogyaPremierModel.selectedTimePeriodModel.year;
  ...
  _showNextButton() {
    onClick() {
      ...
      widget.arogyaPremierModel.policyPeriod = policyPeriod;
      resetData();
      if(widget.arogyaPremierModel.policyType.id == PolicyTypeScreen.INDIVIDUAL){
        Navigator.of(context).pushNamed(ArogyaIndividualMemberDetailsScreen.ROUTE_NAME,
            arguments: widget.arogyaPremierModel);
      }else {
        Navigator.of(context).pushNamed(ArogyaInsureDetailsScreen.ROUTE_NAME,
            arguments: widget.arogyaPremierModel);
      }
  ...
  resetData() async{
    List<PolicyCoverMemberModel> policyMembers = widget.arogyaPremierModel.policyMembers;
    ...
    widget.arogyaPremierModel.policyMembers = policyMembers;
  }
