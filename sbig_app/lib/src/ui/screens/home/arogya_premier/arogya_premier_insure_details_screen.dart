...
import 'arogya_premier_health_question.dart';
import 'arogya_premier_individual_member_details.dart';

class ArogyaInsureDetailsScreen extends StatelessWidget {
  static const ROUTE_NAME = "/arogya/inusree_details_screen";

  final ArogyaPremierModel _arogyaPremierModel;

  ArogyaInsureDetailsScreen(this._arogyaPremierModel);

  @override
  Widget build(BuildContext context) {
    return SbiBlocProvider(
      child: InsureDetailsScreenWidget(_arogyaPremierModel),
...
class InsureDetailsScreenWidget extends StatefulWidgetBase {
  ...
  InsureDetailsScreenWidget(this._arogyaPremierModel);
...
class _InsureDetailsScreenWidgetState extends State<InsureDetailsScreenWidget>
  ...
  @override
  void initState() {
    arogyaPremierModel = widget._arogyaPremierModel;
    _isFamilyFloaterorIndividual = (arogyaPremierModel.policyType.id != 0);
    _bloc = SbiBlocProvider.of<InsureeDetailsBloc>(context);
    policyMembers = arogyaPremierModel.policyMembers;
    if (widget._arogyaPremierModel.policyType.id ==
  ...
  void didChangeDependencies() {
    ...
    PolicyType policyType = arogyaPremierModel.policyType;
                      ...
                      padding: EdgeInsets.only(
                          top: policyMembers.length < 3
                              ? ScreenUtil.getInstance(context).screenHeightDp /
                                  ...
                                  180
                              : 0.0),
                      child: QuickFactWidget(StringDescription.quick_fact17),
                    ),
                    SizedBox(
                      height: 100,
                    ),
  ...
  Widget _showButton() {
    onClick() {
      arogyaPremierModel.policyMembers = policyMembers;
      _makeHealthApiCall();
    }
        ...
        onTap: () {
          if (widget._arogyaPremierModel.policyType.id ==
            ...
            Navigator.popUntil(
                context,
                ModalRoute.withName(
                    ArogyaIndividualMemberDetailsScreen.ROUTE_NAME));
          } else {
            _onSelected(index);
          }
        },
  ...
  _makeHealthApiCall() async {
    ...
    final response = await HealthQuestionApiProvider.getInstance()
        .getHealthQuestionContent(StringConstants.FROM_AROGYA_PREMIER);
    ...
    if (null != response.apiErrorModel) {
     ...
     arogyaPremierModel.healthQuestionResModel = healthQuestionResModel;
      Navigator.of(context).pushNamed(ArogyaHealthQuestion.ROUTE_NAME,
