...
import 'arogya_premier_insurance_buyer_details_screen.dart';

class ArogyaHealthQuestion extends StatefulWidgetBase {
  static const ROUTE_NAME = "/arogya/health_questions_screen";

  final ArogyaPremierModel arogyaPremierModel;

  ArogyaHealthQuestion(this.arogyaPremierModel);

  @override
  _HealthQuestionsScreenState createState() => _HealthQuestionsScreenState();
}

class _HealthQuestionsScreenState extends State<ArogyaHealthQuestion>
    with CommonWidget {
  ...
  String buttonText = "";
  bool isAllPositiveAnswers = true;

  @override
  void initState() {
    textEditingController = TextEditingController();
    HealthQuestionResModel pedQuestionItem =
        widget.arogyaPremierModel.healthQuestionResModel;
  ...
  void didChangeDependencies() {
    List<PolicyCoverMemberModel> _policyMembers =
        widget.arogyaPremierModel.policyMembers;
                    ...
                    child: Center(
                      ...
                      children: <Widget>[
                        SizedBox(
                          width: 30,
                          height: 30,
                          child: (pedQuestionItem.isYes != null &&
                                  pedQuestionItem.isYes)
                              ? (pedQuestionItem.yesActiveIcon == null ||
                                      pedQuestionItem.yesActiveIcon.length == 0)
                                  ? Image.asset(
                                      AssetConstants.ic_medicine_selected)
                                  : Image(
                                      image: NetworkImage(UrlConstants.ICON +
                                          pedQuestionItem.yesActiveIcon),
                                    )
                              : (pedQuestionItem.yesIcon == null ||
                                      pedQuestionItem.yesIcon.length == 0)
                                  ? Image.asset(AssetConstants.ic_medicine)
                                  : Image(
                                      image: NetworkImage(UrlConstants.ICON +
                    ...
                    child: Center(
                      ...
                      children: <Widget>[
                        SizedBox(
                          ...
                          child: (pedQuestionItem.isYes != null &&
                                  !pedQuestionItem.isYes)
                              ? (pedQuestionItem.noActiveIcon == null ||
                                      pedQuestionItem.noActiveIcon.length == 0)
                                  ? Image.asset(
                                      AssetConstants.ic_medicine_white_selected)
                                  : Image(
                                      image: NetworkImage(UrlConstants.ICON +
                                          pedQuestionItem.noActiveIcon),
                                    )
                              : (pedQuestionItem.noIcon == null ||
                                      pedQuestionItem.noIcon.length == 0)
                                  ? Image.asset(
                                      AssetConstants.ic_medicine_white)
                                  : Image(
                                      image: NetworkImage(UrlConstants.ICON +
...
//                },
  ...
  onClick() {
    try {
      ...
      setState(() {
        isAlertVisible = false;
        isAllPositiveAnswers = true;
        for (int i = 1; i < pedQuestionsList.length; i++) {
          if (pedQuestionsList[i].isYes) {
            isAllPositiveAnswers = false;
          }
        }
      });

      PersonalDetails personalDetails =
          widget.arogyaPremierModel.personalDetails;
      List<PolicyCoverMemberModel> policyMembers =
          widget.arogyaPremierModel.policyMembers;
      ArogyaTimePeriodModel timePeriodModel =
          widget.arogyaPremierModel.selectedTimePeriodModel;
      ArogyaSumInsuredModel sumInsuredModel =
          widget.arogyaPremierModel.selectedSumInsured;
      PolicyPeriod policyPeriod = widget.arogyaPremierModel.policyPeriod;
      DateTime startDate = policyPeriod.startDate;
      DateTime endDate = policyPeriod.endDate;
      int policyType = widget.arogyaPremierModel.policyType.id;

      List<PolicyRiskList> list = [];

      ArogyaPremierQuickQuoteReqModel arogyaPremierQuickQuoteReqModel =
          ArogyaPremierQuickQuoteReqModel();
      arogyaPremierQuickQuoteReqModel.arogyaPolicyType =
          StringConstants.AROGYA_PREMIER;
      arogyaPremierQuickQuoteReqModel.mobile = personalDetails.mobile;
      arogyaPremierQuickQuoteReqModel.email = personalDetails.email;
      arogyaPremierQuickQuoteReqModel.policyDuration =
          timePeriodModel.year.toString();
      if (widget.arogyaPremierModel.policyType.id ==
          PolicyTypeScreen.INDIVIDUAL) {
        arogyaPremierQuickQuoteReqModel.policyType = "individual";
      } else if (widget.arogyaPremierModel.policyType.id ==
          PolicyTypeScreen.FAMILY_FLOATER) {
        arogyaPremierQuickQuoteReqModel.policyType = "family_floater";
      } else if (widget.arogyaPremierModel.policyType.id ==
          PolicyTypeScreen.FAMILY_INDIVIDUAL) {
        arogyaPremierQuickQuoteReqModel.policyType = "family_individual";
      }
      arogyaPremierQuickQuoteReqModel.effectiveDate =
          CommonUtil.instance.convertTo_yyyy_MM_dd(startDate);
      arogyaPremierQuickQuoteReqModel.expiryDate =
      ...
      for (PolicyCoverMemberModel item in policyMembers) {
        ...
        if (member.relation.toLowerCase().startsWith("child")) {
          policyRiskList.argInsuredRelToProposer =
              member.ageGenderModel.gender == "M" ? "Son" : "Daughter";
        } else if (member.relation
                .toLowerCase()
                .compareTo(S.of(context).father_in_law_title.toLowerCase()) ==
            0) {
          policyRiskList.argInsuredRelToProposer = "father_in_law";
        } else if (member.relation
                .toLowerCase()
                .compareTo(S.of(context).mother_in_law_title.toLowerCase()) ==
            0) {
        ...
        for (PedQuestionsModel pedQuestionsModel in pedQuestionsList) {
          ...
          List<PolicyCoverMemberModel> policyMembers =
              pedQuestionsModel.arogyaMembers;
          if (selectedMembers.isEmpty &&
              (policyRiskList.argInsuredRelToProposer.compareTo("Self") == 0 ||
            ...
            for (int i in selectedMembers) {
              if (policyMembers[i]
                ...
                switch (j) {
                  case 0:
                    policyRiskList.otherImpairment = "1";
                    policyRiskList.otherImpairmentReason =
      ...
      arogyaPremierQuickQuoteReqModel.policyRiskList = list;
      doApiCall(arogyaPremierQuickQuoteReqModel, pedQuestionsList[0].isYes,
          policyType);
    } catch (e) {
  ...
  void doApiCall(ArogyaPremierQuickQuoteReqModel recalculatePremium,
      bool isFirstQuestionyes, int policyType) async {
/*
    widget.arogyaPremierModel.isPremiumFrom = PremiumBreakupScreen.FROM_HEALTH_QUESTIONNAIRE;
    if(Platform.isIOS){
      showCupertinoDialog(
          context: context,
          builder: (BuildContext context) => WillPopScope(
              ...
              child: ArogyaPremierPremiumBreakupScreen(widget.arogyaPremierModel)));
    }else{
      showDialog(
          context: context,
          builder: (BuildContext context) => WillPopScope(
...
*/
    ...
    var response = await ArogyaPremierPremiumApiProvider.getInstance()
        .calculateQuickQuote(body, policyType);
    if (null == response.apiErrorModel) {
      ...
      if (isFirstQuestionyes) {
        ...
        ArogyaTimePeriodModel timePeriodModel =
            widget.arogyaPremierModel.selectedTimePeriodModel;
        ArogyaPremierQuickQuoteResModel resModel = response;
        timePeriodModel.totalPremium =
            resModel?.data?.premiumDetails?.totalPremium ?? '0';
        timePeriodModel.actualPremium =
            resModel?.data?.premiumDetails?.basePremium ?? '0';
        ...
        widget.arogyaPremierModel.isPremiumFrom =
            PremiumBreakupScreen.FROM_HEALTH_QUESTIONNAIRE;
        widget.arogyaPremierModel.selectedTimePeriodModel = timePeriodModel;
        ...
        widget.arogyaPremierModel.arogyaPremierQuickQuoteReqModel =
            recalculatePremium;
        widget.arogyaPremierModel.arogyaPremierQuickQuoteResModel = resModel;

        if (isAllPositiveAnswers) {
          Navigator.of(context).pushNamed(
              ArogyaPremierInsuranceBuyerDetailsScreen.ROUTE_NAME,
          ...
          if (Platform.isIOS) {
            showCupertinoDialog(
                ...
                builder: (BuildContext context) => WillPopScope(
                    ...
                    child: ArogyaPremierPremiumBreakupScreen(
                        widget.arogyaPremierModel)));
          } else {
            showDialog(
                context: context,
                builder: (BuildContext context) => WillPopScope(
  ...
  Widget buildMemberListItem(
      PolicyCoverMemberModel policyMember, int subIndex, int mainIndex) {
    bool isSelected =
