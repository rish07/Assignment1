...
import 'arogya_premier_sum_insured_screen.dart';
import 'arogya_time_period_screen.dart';

class ArogyaCoverMemberScreen extends StatelessWidget{
  static const ROUTE_NAME = "/arogya_policy_cover_member_screen";
  final ArogyaPolicyCoverMemberArguments arguments;

  ArogyaCoverMemberScreen(this.arguments);
...
class _State extends State<ArogyaPolicyCoverMemberScreenWidget> with CommonWidget {
  ...
  Widget buildListItem(PolicyCoverMemberModel policyMember, int index) {
    ...
    String title = '';
    if(policyMember.title.compareTo(S.of(context).self_title)==0 && policyType != PolicyTypeScreen.INDIVIDUAL){
  ...
  _onSelected(int index) {
    setState(() {
     ...
     if(policyType == PolicyTypeScreen.FAMILY_INDIVIDUAL){
       ...
       print('_model ${_model.sumInsuredResModel?.data?.length},INDEX $index');
       _showMemberDetailsDialog(member,_model);
     }else{
       ArogyaMemberDetailsBottomSheetWidget(
  ...
  void _showMemberDetailsDialog(PolicyCoverMemberModel member, ArogyaFamilyIndividualModel model) {
      if (Platform.isIOS) {
      showCupertinoDialog(
          context: context,
          builder: (BuildContext context) => WillPopScope(
              ...
              child:  MemberDetailsFamilyIndividualBottomSheetWidget(
                 _onUpdateFamilyIndividual, member.memberDetailsModel, model ,_selectedIndex,
              StringConstants.FROM_AROGYA_PREMIER)));
    } else {
      showDialog(
          context: context,
          builder: (BuildContext context) => WillPopScope(
              ...
              child:  MemberDetailsFamilyIndividualBottomSheetWidget(
                  _onUpdateFamilyIndividual,  member.memberDetailsModel, model ,_selectedIndex, StringConstants.FROM_AROGYA_PREMIER)));
    }
  }

  _onUpdate(int age, String gender, int index, bool remove) {

    print('*****************   Age : $age , Gender : $gender, index : $index , remove : $remove ');
  ...
  _onUpdateFamilyIndividual(int index , bool remove , ArogyaFamilyIndividualModel arogyaFamilyIndividualModel) {

    print('Age : ${arogyaFamilyIndividualModel.age} , Gender : ${arogyaFamilyIndividualModel.gender}, index : $index , remove : $remove , SumInsuredResModel:${arogyaFamilyIndividualModel.sumInsuredResModel.data.length},  _premium :${arogyaFamilyIndividualModel.premium}');
    ...
    if (remove == true && selectedMembers.contains(index)) {
      ...
      if (arogyaFamilyIndividualModel.age == null || TextUtils.isEmpty(arogyaFamilyIndividualModel.gender.toString())|| arogyaFamilyIndividualModel.sumInsuredResModel ==null || arogyaFamilyIndividualModel.sumInsured<=0) {
        isAllAvailable = false;
      }
      if (isAllAvailable) {
        ageGenderModel.age = arogyaFamilyIndividualModel.age;
        ageGenderModel.gender = arogyaFamilyIndividualModel.gender;
        sumInsuredResModel =arogyaFamilyIndividualModel.sumInsuredResModel;
      if(policyCoverMemberModelList[_selectedIndex].memberDetailsModel.relation.compareTo(S.of(context).self_title)==0){
        selectedSumInsuredModel=ArogyaSumInsuredModel(
          amount: arogyaFamilyIndividualModel.sumInsured,
          amountString: CommonUtil.instance.convertSumInsured(arogyaFamilyIndividualModel.sumInsured),
          premium: arogyaFamilyIndividualModel.premium,
          isSelected: true,
        );
      }
      ...
      setState(() {
        policyCoverMemberModelList[_selectedIndex].memberDetailsModel.ageGenderModel = ageGenderModel;
        arogyaFamilyIndividualList[_selectedIndex]=arogyaFamilyIndividualModel;
      });
          ...
          switch (policyType) {
            case PolicyTypeScreen.INDIVIDUAL:
              ...
              if (member.relation.compareTo(S.of(context).self_title) == 0) {
                arogyaPremierModel.isProposerSelf = true;
              } else {
                arogyaPremierModel.isProposerSelf = false;
              }

              _sumInsuredApiCall(ageGenderModel.age,arogyaPremierModel: arogyaPremierModel);
              break;
            case PolicyTypeScreen.FAMILY_FLOATER:
              if (selectedMembers.length < 2) {
                ...
                for (int index in selectedMembers) {
                  ...
                  if (memberDetailsModel.relation.compareTo(S.of(context).self_title) == 0) {
                    arogyaPremierModel.isProposerSelf = true;
                  } else {
                    arogyaPremierModel.isProposerSelf = false;
                  }
                ...
                _sumInsuredApiCall(maxAge,arogyaPremierModel: arogyaPremierModel);
              }
              break;
            case PolicyTypeScreen.FAMILY_INDIVIDUAL:
              if (selectedMembers.length < 2) {
                ...
                for (int index in selectedMembers) {
                  MemberDetailsModel memberDetailsModel = policyCoverMemberModelList[index].memberDetailsModel;
                  AgeGenderModel ageGenderModel = memberDetailsModel.ageGenderModel;
                  ArogyaFamilyIndividualModel arogyaFamilyIndividualModel = arogyaFamilyIndividualList[index];

                  if (ageGenderModel.gender == null && ageGenderModel.age == null && memberDetailsModel.sumInsuredString == null) {
                    isNextButtonEnable=false;
                    return;
                  } else if (ageGenderModel.gender == null) {
                    isNextButtonEnable=false;
                  ...
                  if(selfSumInsured < arogyaFamilyIndividualModel.sumInsured){
                    setState(() {
                      isAlertVisible=true;
                      errorString=S.of(context).sum_insured_family_individual_error;
                    });
                    return;
                  }
                  isNextButtonEnable=true;
                  ...
                  if(memberDetailsModel.relation.compareTo(S.of(context).self_title) == 0){
                    arogyaPremierModel.sumInsuredResModel = arogyaFamilyIndividualModel.sumInsuredResModel;
                  }

                  if (memberDetailsModel.relation.compareTo(S.of(context).self_title) == 0 ||
                      memberDetailsModel.relation.compareTo(S.of(context).spouse_title) == 0||
                      memberDetailsModel.relation.compareTo(S.of(context).father_title)==0||
                          memberDetailsModel.relation.compareTo(S.of(context).mother_titile)==0||
                      memberDetailsModel.relation.compareTo(S.of(context).father_in_law_title) == 0||
                      memberDetailsModel.relation.compareTo(S.of(context).mother_in_law_title)==0) {
                    adultCount++;
                  }

                  if (memberDetailsModel.relation.compareTo(S.of(context).child1_title) == 0 || memberDetailsModel.relation
                      .compareTo(S.of(context).child2_title) ==
                      0 || memberDetailsModel.relation
                      .compareTo(S.of(context).child3_title) ==
                      0 ||
                  ...
                  membersList.add(Members(age: ageGenderModel.age,sumInsured: arogyaFamilyIndividualModel.sumInsured));
                }
                ...
                arogyaPremierPremiumReqModel.memberCount = selectedMembers.length ?? 0;

                _makeApiCall(arogyaPremierPremiumReqModel,arogyaPremierModel: arogyaPremierModel);
              }
              break;
          }
        }
  ...
  _makeApiCall(ArogyaPremierPremiumReqModel arogyaPremierPremiumReqModel, {ArogyaPremierModel arogyaPremierModel}) async {
    showLoaderDialog(context);
    final response = await ArogyaPremierPremiumApiProvider.getInstance().calculatePremium(arogyaPremierPremiumReqModel.toJson(),  (policyType== PolicyTypeScreen.INDIVIDUAL) ? false : true);
    hideLoaderDialog(context);
    if (response != null) {
      ArogyaSumInsuredModel _selectedSumInsuredModel = selectedSumInsuredModel;
      _selectedSumInsuredModel.arogyaPremierPremiumReqModel = arogyaPremierPremiumReqModel;
      _selectedSumInsuredModel.arogyaPremierPremiumResModel = response;
      _selectedSumInsuredModel.timePeriod = response?.data ?? null;
      arogyaPremierModel.selectedSumInsured = selectedSumInsuredModel;
  ...
  void _sumInsuredApiCall(int age,{ArogyaPremierModel arogyaPremierModel}) async {
    var response = await _coverMemberBloc.getSumInsured(isFrom, age);
    if (response != null) {
      arogyaPremierModel.sumInsuredResModel = response;
      arogyaPremierModel.policyType = policyTypeObj;
      Navigator.of(context).pushNamed(ArogyaPremierSumInsuredScreen.ROUTE_NAME,
          arguments: ArogyaSumInsuredArguments(arogyaPremierModel));
    }
  }
      ...
      if (data != null) {
        for (int i = 0; i < data.length; i++) {
          ...
          if (data[i].slugType.contains('button')) {
            ...
            if(tit.contains('Child') && (isFrom == StringConstants.FROM_AROGYA_PREMIER|| isFrom == StringConstants.FROM_AROGYA_TOP_UP) ){
...
class ArogyaPolicyCoverMemberArguments {
  final PersonalDetails personalDetails;
  final int policyType;
  final CoverMemberResModel resModel;

  ArogyaPolicyCoverMemberArguments(this.personalDetails, this.resModel,{this.policyType,});
}
